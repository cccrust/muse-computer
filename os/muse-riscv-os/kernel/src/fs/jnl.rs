...
//   slot s (0..NREC): sector A = {MAGIC_A, gen, seq, lba, crc32(seq,lba,data)},
//                      sector B = 512B data.
...
// - record complete     -> replay (idempotent; same-lba ordered by seq)
...
// victim's home was already applied (write-through ordering).
...
/// Binds (seq, lba, data) so torn mixes of slot generations can never
/// validate: any partial overwrite fails.
...
static JLOG: crate::sync::SpinMutex<JLog> = crate::sync::SpinMutex::new(JLog { next: 0, gen: 0 });
...
fn max_seq(gen: u32) -> u32 {
    ...
    for s in 0..n {
        ...
        if r32(&a, 0) != MAGIC_A || r32(&a, 4) != gen {
            continue;
        }
        let seq = r32(&a, 8);
        if seq == 0 {
...
/// write happens after we return (caller). Complete in ~3 raw sector
...
pub fn record(lba: u32, data: &[u8; 512]) {
    ...
    w32(&mut a, 8, seq);
    w32(&mut a, 12, lba);
    w32(&mut a, 16, jnl_crc(seq, lba, data));
    raw_write(base, &a);
    raw_write(base + 1, data);
}

/// Replay all valid records of the current generation in seq order.
/// Idempotent: safe to run on every dirty mount. Returns (applied, max).
pub fn replay() -> (usize, u32) {
    let n = nrec().min(NREC);
    let gen = JLOG.lock().gen;
    let mut recs: Vec<(u32, u32, [u8; 512])> = Vec::new();
    for s in 0..n {
        let mut a = [0u8; 512];
        raw_read(slot_base(s), &mut a);
        if r32(&a, 0) != MAGIC_A || r32(&a, 4) != gen {
