...
// v3.4: host `cargo test` builds this crate with std (test harness +
// NIST vectors below). Riscv target builds never define test, so the
// guest keeps pure no_std. The raw `ecall` asm exists only on riscv64
// ("ecall" is not valid host asm and would never execute there anyway).
#[cfg(test)]
extern crate std;
...
/// Non-riscv stub: syscall wrappers link on host (for sha256 tests)
// without ever executing a trap.
#[cfg(not(target_arch = "riscv64"))]
#[inline(always)]
fn ecall(_id: usize, _a0: usize, _a1: usize, _a2: usize) -> isize {
    panic!("ecall on non-riscv host");
}
...
/// v3.5: bind host dir `tgt` at `mp` for the caller (SYS_MOUNTVOL=56).
/// Both sides resolve in the caller's current view. Returns 0 or -1
...
// ---- v3.4: SHA-256 (core-only, no alloc; FIPS 180-4) ----
...
/// Streaming SHA-256 (init/update/finalize; caller feeds any chunking).
/// Backs package integrity checks (v3.4: hash concatenated layer bytes).
pub struct Sha256 {
    h: [u32; 8],
    len: u64, // total input bytes (for the length suffix)
    buf: [u8; 64],
    bufn: usize,
}
...
impl Sha256 {
    ...
    pub fn update(&mut self, mut data: &[u8]) {
        ...
        if self.bufn > 0 {
            let take = (64 - self.bufn).min(data.len());
            self.buf[self.bufn..self.bufn + take].copy_from_slice(&data[..take]);
            self.bufn += take;
            data = &data[take..];
            if self.bufn == 64 {
                let mut b = [0u8; 64];
...
/// lowercase hex of a hash (64 bytes, no NUL).
pub fn sha256_hex(sum: &[u8; 32], out: &mut [u8; 64]) {
    let mut i = 0;
    while i < 32 {
        let hb = b"0123456789abcdef";
        out[i * 2] = hb[(sum[i] >> 4) as usize];
        out[i * 2 + 1] = hb[(sum[i] & 15) as usize];
        i += 1;
    }
}
...
mod tests {
    ...
    fn hex_of(data: &[u8]) -> [u8; 64] {
        let mut out = [0u8; 64];
        sha256_hex(&sha256(data), &mut out);
        out
    }
    ...
    fn sha256_multiblock() {
        // 112-byte NIST-adjacent vector (two blocks after padding).
        let v = b"abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq";
        assert_eq!(
            &hex_of(v),
    ...
    #[test]
    fn sha256_streaming_split() {
        // chunked feeding (odd splits incl. 63/65 around the block edge)
        // must equal one-shot.
        let v = b"The quick brown fox jumps over the lazy dog. Pack my box!";
        let one = sha256(v);
        for &at in &[0usize, 1, 55, 63, 64, 65, 100] {
            let mut st = Sha256::new();
            st.update(&v[..at.min(v.len())]);
            st.update(&v[at.min(v.len())..]);
            assert_eq!(st.finalize(), one, "split at {}", at);
        }
    }
}
