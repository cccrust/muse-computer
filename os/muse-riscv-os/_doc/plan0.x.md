...
> 狀態：v0.1 已結案（見 `_doc/v0.1.md`），v0.2 已結案（見 `_doc/v0.2.md`：磁碟 FS + 雙啟動持久化測試全過），
> v0.3 已結案（見 `_doc/v0.3.md`：argv + link + `&` + kill + fsck-lite）。
> v0.4 進行中（見 `_doc/v0.4.md`：中斷驅動 I/O）。
> 本計畫第 1–9 節為原始規劃，實際落點與取捨見文末 §10；
